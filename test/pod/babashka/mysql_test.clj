...
(pods/load-pod (if true #_(= "native" (System/getenv "POD_TEST_ENV"))
                 "./pod-babashka-mysql"
...
(deftest mysql-test
;  (db/execute! db ["create database test"])
  (db/execute! db ["drop table if exists foo"])
  (db/execute! db ["drop table if exists foo_timed"])
  (db/execute! db ["drop table if exists java_time"])
  ...
  (testing "transaction"
    ...
    (testing "with-transaction"
      ...
      (testing "reusing connection"
        (let [conn (db/get-connection db)]
         (dotimes [_ 1]
           (is (= [#:next.jdbc{:update-count 2}]
                 (db/with-transaction [x conn]
                   (try (db/execute! x ["xxx"])
                        (catch Exception _))
                   (db/execute! x ["insert into foo values (5);"])
                   (db/execute! x ["insert into foo values (6), (7);"])))))))
